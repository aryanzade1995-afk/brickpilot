...
import { CaptureCanvas } from '@/lib/render/CaptureCanvas.tsx'
...
export function Render() {
  ...
  // rasterize the marked interior plan as the fourth reference
  useEffect(() => {
    if (phase !== 'capturing' || svgBusy.current || !svgRef.current) return
    svgBusy.current = true
    rasterizeSvg(svgRef.current)
      .then((url) => setRef('interior', url))
      .catch((e) => captureFailed(String(e.message)))
      .finally(() => {
        svgBusy.current = false
      })
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-8 md:px-10">
              ...
              <figure key={k} className="space-y-1.5">
                <div className="relative flex aspect-[3/2] items-center justify-center overflow-hidden border border-line-strong bg-bg-inset">
                  {refs[k] ? (
                    ...
                    <Camera size={14} className="text-ink-faint" />
                  )}
                </div>
      ...
      {phase === 'capturing' && <CaptureCanvas design={design} character={character} />}
